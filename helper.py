...
def most_busy_users(df):
    ...
    df = round((df['user'].value_counts() / df.shape[0])*100 , 2).reset_index().rename(columns = {'index': 'Name' , 'users': 'percent'})
    return x , df
...
def create_word_cloud(selected_user , df):
    ...
    temp_df['message'] = temp_df['message'].apply(remove_stopword)
